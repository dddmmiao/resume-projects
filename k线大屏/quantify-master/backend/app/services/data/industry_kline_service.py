...
class IndustryKlineService(BaseKlineService):
    """行业指数K线数据服务类"""

    def __init__(self):
        super().__init__(EntityTypes.INDUSTRY)
        self.data_service = tushare_service
        logger.info("行业K线服务初始化完成")
    ...
    def _get_industry_kline_data_full(
            self,
            ts_code: str,
            period: str = "daily",
            use_cache: bool = True,
        ...
        """
        try:
            logger.debug(f"获取行业K线数据 - ts_code: {ts_code}, period: {period}")
    ...
    def get_industry_kline_data(
        ...
        """
        获取行业指数K线数据（按 limit 切片，转换为Pydantic模型，过滤指标字段）。
        ...
        # 获取完整数据（包含指标字段，使用缓存）
        data = self._get_industry_kline_data_full(ts_code=ts_code, period=period)
        ...
        return KlineQueryUtils.convert_kline_data_to_models(data, TableTypes.INDUSTRY)

    # ============== 指标数据（直接使用K线数据缓存，避免重复缓存） ==============
    def _get_industry_indicators_full(self, ts_code: str, period: str = "daily") -> List[Dict[str, Any]]:
        """
        获取行业指标数据（直接使用K线数据缓存，因为K线数据已包含所有指标字段）
        ...
        try:
            ...
            logger.error(f"获取行业指标数据失败: ts_code={ts_code}, period={period}, error={e}")
            return []

    def get_industry_indicators_cached(self, ts_code: str, period: str = "daily", limit: int = 100, end_date: Optional[str] = None) -> List[
        Dict[str, Any]]:
        data = self._get_industry_indicators_full(ts_code=ts_code, period=period)
    ...
    def batch_get_industry_indicators_cached(self, ts_codes: List[str], period: str = "daily", limit: int = 100, end_date: Optional[str] = None) -> \
    Dict[str, List[Dict[str, Any]]]:
        """批量获取行业指标数据（基于原有缓存方案优化）"""
        if not ts_codes:
            return {}
        ...
        def fetch_single(code: str):
            try:
                data = self.get_industry_indicators_cached(ts_code=code, period=period, limit=limit, end_date=end_date)
                return (code, data or [])
            except Exception as e:
                logger.debug(f"获取行业指标数据失败: {code}, {e}")
                return (code, [])
    ...
    def _fetch_kline_from_tushare(
        ...
        try:
            ...
            logger.error(f"从Tushare获取行业K线数据失败: {e}")
    ...
    def _bulk_store_data(
        ...
        try:
            from ...dao.industry_kline_dao import industry_kline_dao

            result = industry_kline_dao.bulk_upsert_industry_kline_data(
                data=data,
            ...
            total_duration = time.time() - start_time
            logger.error(f"❌ 行业K线存储失败 | 耗时: {total_duration:.2f}s | 错误: {e}")
            return {"inserted_count": 0, "updated_count": 0}

    def _invalidate_cache(self, period: str, codes: List[str]):
        """失效行业缓存"""
        from ..core.cache_service import cache_service
        cache_service.invalidate_industry_klines_for_codes(period, codes)

    def _get_kline_data_full_method(self, period: str):
        """获取带缓存的K线数据方法（用于预热）"""
        return lambda code: self._get_industry_kline_data_full(code, period)
    ...
    def create_kline_sync_tasks(
        ...
        try:
            ...
            if not all_selected and not codes:
                raise ValidationException("请选择要同步的行业或使用全选")
            ...
            req = SchedulerService.UnifiedKlineSyncRequest(
                subject_type=EntityTypes.INDUSTRY,
            ...
            return {
                "success": True,
                "message": f"行业{period_display}同步任务已创建",
                "task_execution_id": result["task_execution_id"],
            }
        except (ValidationException, DatabaseException):
            raise
        except Exception as e:
            logger.error(f"创建行业同步任务失败: {e}")
            raise DatabaseException(str(e))


# 创建全局服务实例
industry_kline_service = IndustryKlineService()
