"""
概念数据访问层 (DAO) - SQLModel优化版本
负责概念板块及相关关联数据的数据库操作，提供高性能的查询和批量操作
"""
...
class ConceptDAO:
    ...
    @staticmethod
    def bulk_upsert_stock_concept_data(
            data: List[Dict[str, Any]],
            batch_size: Optional[int] = None
    ) -> Dict[str, int]:
        """
    ...
    def get_concepts(
        ...
        Args:
            search: 搜索关键词
            search_fields: 搜索字段列表，默认["concept_name"]
            limit: 限制数量
            offset: 偏移量
    ...
    def get_concepts_smart(
        ...
        """
        try:
            logger.info(f"概念DAO查询（基础表+K线表） - filters: {filters}, search: {search}, sort_by: {sort_by}, sort_period: {sort_period}")
            ...
            logger.info(f"概念DAO查询结果（基础表+K线表） - 数据量: {len(result.get('data', []))}, 总数: {result.get('total', 0)}")
            return result
        except Exception as e:
            logger.error(f"get_concepts_smart 查询失败: {e}")
    ...
    def get_filtered_concept_codes(
        ...
        try:
            result = query_utils.get_records_smart(
                ...
                search=search,
                search_fields=search_fields,
                sort_by=sort_by or "concept_code",
                sort_order=sort_order,
                sort_period=sort_period,
                limit=limit,
                offset=0,
            ...
            return [item.get("concept_code") for item in result.get("data", []) if item.get("concept_code")]
        except Exception as e:
            logger.error(f"get_filtered_concept_codes 查询失败: {e}")
            return []


    @staticmethod
    def get_concept_stats_aggregated(
        filters: Optional[Dict[str, Any]] = None,
        search: Optional[str] = None,
        search_fields: Optional[List[str]] = None,
        trade_date: Optional[str] = None,
        ...
        if not trade_date:
            logger.warning("get_concept_stats_aggregated 未提供 trade_date，返回空统计结果")
        ...
        # 获取K线表模型
        kline_model = TableFactory.get_table_model(TableTypes.CONCEPT, year)
        if kline_model is None:
            logger.warning(f"未找到年份 {year} 的概念K线表模型")
            return empty_payload
        
        try:
            with db_session_context() as db:
                concept_model, _ = TableTypes.get_model_info(TableTypes.CONCEPT)
                m = kline_model
                
                from sqlalchemy import exists
                
                # 构建基础表的筛选条件子查询
                base_exists_subq = select(concept_model.concept_code).where(
                    concept_model.concept_code == m.ts_code
                ).correlate(m)
                
                # 应用筛选和搜索条件
                base_exists_subq = ConceptDAO._apply_filter_conditions(
                    base_exists_subq, filters, search, search_fields, concept_model
                )
                
                # 查询所有概念明细数据
                items_query = select(
                    m.ts_code,
                    concept_model.concept_name,
                    m.close,
                    m.open,
                    m.pct_chg,
                    m.amount,
                    ...
                    concept_model, m.ts_code == concept_model.concept_code
                ).where(
            ...
            logger.error(f"get_concept_stats_aggregated 统计失败: {e}")
            return empty_payload
    ...
    def get_concept_compare_stats(
        ...
        if not base_date or not compare_date:
            logger.warning("get_concept_compare_stats 缺少 base_date 或 compare_date")
            return empty_payload
        ...
        base_kline_model = TableFactory.get_table_model(TableTypes.CONCEPT, base_year)
        compare_kline_model = TableFactory.get_table_model(TableTypes.CONCEPT, compare_year)
        
        if base_kline_model is None or compare_kline_model is None:
            logger.warning(f"未找到年份 {base_year} 或 {compare_year} 的概念K线表模型")
            return empty_payload
        ...
        try:
            with db_session_context() as db:
                ...
                # 构建筛选后的概念子查询
                filtered_concepts = select(concept_model.concept_code, concept_model.concept_name)
                filtered_concepts = ConceptDAO._apply_filter_conditions(
                    filtered_concepts, filters, search, search_fields, concept_model
                )
                filtered_concepts_subq = filtered_concepts.subquery()
                ...
                if base_year == compare_year:
                    ...
                    # 构建 ts_code IN (...) 子查询
                    ts_codes_subq = select(filtered_concepts_subq.c.concept_code)
                    
                    # 🚀 查询1：只查询 A日 和 B日 两天数据（快速）
                    price_query = select(
                        k.ts_code,
                        filtered_concepts_subq.c.concept_name,
                        ...
                        filtered_concepts_subq, k.ts_code == filtered_concepts_subq.c.concept_code
                    ).where(
                        k.ts_code.in_(ts_codes_subq),
                        k.period == sort_period,
                        k.trade_date.in_([base_dt, compare_dt])
                    ).group_by(k.ts_code, filtered_concepts_subq.c.concept_name)
                ...
                price_a_query = select(
                    ...
                    filtered_concepts_subq, k_a.ts_code == filtered_concepts_subq.c.concept_code
                ).where(
                ...
                price_b_query = select(
                    ...
                    filtered_concepts_subq, k_b.ts_code == filtered_concepts_subq.c.concept_code
                ).where(
            ...
            logger.error(f"get_concept_compare_stats 统计失败: {e}")
            return empty_payload

    @staticmethod
    def get_all_ts_codes() -> List[Dict[str, Any]]:
        """返回全部概念代码，统一字段名为 ts_code - SQLModel优化版本
        
        Note: 此方法返回相对静态的数据，建议在Service层添加缓存
        """
        try:
            # 🚀 SQLModel优化：使用上下文管理器，自动管理连接
            with db_session_context() as db:
                stmt = select(Concept.concept_code)
                result = db.exec(stmt).all()
                return [{"ts_code": code} for code in result if code]
        except Exception as e:
            logger.warning(f"查询概念全部 ts_code 失败: {e}")
            return []

    @staticmethod
    def get_hot_concept_codes() -> List[str]:
        """
        ...
        try:
            with db_session_context() as db:
                stmt = select(Concept.concept_code).where(
                    Concept.hot_rank.isnot(None)
                ).order_by(Concept.hot_rank.asc())
                result = db.exec(stmt).all()
                return list(result)
        except Exception as e:
    ...
    @staticmethod
    def load_stock_concepts(ts_code: str) -> List[str]:
        """
        从数据库加载股票的概念关联（返回概念名称数组）- SQLModel优化版本
        ...
        """
        try:
            # 🚀 SQLModel优化：使用JOIN查询获取概念名称而不是代码
            with db_session_context() as db:
                stmt = select(Concept.concept_name).join(
                    StockConcept, Concept.concept_code == StockConcept.concept_code
                ).where(
                    StockConcept.ts_code == ts_code
                )
                result = db.exec(stmt).all()
                return list(result)
        except Exception as e:
            logger.warning(f"加载股票概念关联失败 ({ts_code}): {e}")
            return []

    @staticmethod
    def get_ts_codes_by_concept_codes(concept_codes: List[str]) -> List[str]:
        """
        根据概念代码集合获取关联的股票 ts_code 列表 - SQLModel优化版本
        """
        ...
        try:
            ...
            with db_session_context() as db:
                stmt = select(StockConcept.ts_code).where(
                    StockConcept.concept_code.in_(concept_codes)
                ).distinct()
                result = db.exec(stmt).all()
                return list(result)
        except Exception as e:
            logger.warning(f"查询概念关联股票失败: {e}")
            return []

    @staticmethod
    def get_concept_codes_by_stock_codes(stock_codes: List[str]) -> List[str]:
        """
        根据股票代码集合获取关联的概念代码列表 - SQLModel优化版本
        """
        if not stock_codes:
            return []
        ...
        try:
            # 🚀 SQLModel优化：使用上下文管理器和select查询
            with db_session_context() as db:
                stmt = select(StockConcept.concept_code).where(
                    StockConcept.ts_code.in_(stock_codes)
                ).distinct()
                result = db.exec(stmt).all()
                return [code for code in result if code]
        except Exception as e:
            logger.warning(f"查询股票关联概念失败: {e}")
            return []
    ...
    def sync_hot_data(
        ...
        try:
            ...
            for hot_item in hot_data_list:
                concept_code = hot_item.get("concept_code")
                if not concept_code:
                    continue
            ...
            stats = ConceptDAO._bulk_update_hot_data(update_data)
    ...
    def _bulk_update_hot_data(update_data: List[Dict[str, Any]]) -> Dict[str, int]:
        """
        批量更新概念热度数据（只更新，不插入）- SQLModel优化版本
        """
        if not update_data:
            return DAOConfig.format_upsert_result({
                "inserted": 0,
        ...
        try:
            ...
            with db_session_context() as db:
                # 过滤掉无效的concept_code
                valid_data = [data for data in update_data if data.get('concept_code')]
